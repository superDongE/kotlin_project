...
class QuestionAdapter3(val context: Context, val studyList: ArrayList<QuestionResponseResult>, val apply: Int) :
    RecyclerView.Adapter<QuestionAdapter3.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return QuestionAdapter3.ViewHolder(
            LayoutInflater.from(context).inflate(R.layout.lastadpatertest, parent, false)
        )
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {

        holder.title?.text = studyList[position+12].choice
